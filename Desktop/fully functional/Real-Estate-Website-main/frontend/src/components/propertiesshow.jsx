...
/* ---------- BDT formatter (English digits) ----------
   If you want Bengali digits instead, use:
   const formatBDT = n => '৳ ' + new Intl.NumberFormat('bn-BD').format(Number(n||0));
----------------------------------------------------- */
const formatBDT = (n) => "৳ " + Number(n || 0).toLocaleString("en-US");

/* ---------- Bangladesh sample properties (fallback) ---------- */
const sampleProperties = [
  {
    ...
    availability: "Buy",
    image: [
      "https://images.unsplash.com/photo-1613490493576-7fde63acd811?q=80&w=1350&auto=format&fit=crop",
    ],
  },
  {
    _id: "sample2",
    title: "Banani Modern Apartment",
    ...
    availability: "Rent",
    image: [
      "https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?q=80&w=1350&auto=format&fit=crop",
    ],
  },
  {
    _id: "sample3",
    title: "Sea-View Townhouse",
    ...
    availability: "Buy",
    image: [
      "https://images.unsplash.com/photo-1512917774080-9991f1c4c750?q=80&w=1350&auto=format&fit=crop",
    ],
  },
];
...
const PropertyCard = ({ property }) => {
  ...
  const handleNavigate = () => navigate(`/properties/single/${property._id}`);

  const toggleFavorite = (e) => {
    e.stopPropagation();
    setIsFavorite((p) => !p);
    // TODO: save favorite via API if needed
  };
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center text-blue-600 font-bold">
            <span className="text-xl">{formatBDT(property.price)}</span>
          </div>
...
const PropertiesShow = () => {
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        ...
        if (res.data?.success && Array.isArray(res.data.property)) {
          setProperties(res.data.property.slice(0, 6));
        } else {
  ...
  const viewAll = () => navigate("/properties");
  ...
  return (
    <section className="mt-20 md:mt-28 py-24 bg-gradient-to-b from-white to-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          {["all", "apartment", "villa", "house"].map((id) => (
            <button
              key={id}
            ...
            >
              {id === "all" ? "All Properties" : id[0].toUpperCase() + id.slice(1) + "s"}
            </button>
          ))}
        </motion.div>

...
const Hero = () => {
  ...
  const handleSubmit = (location = searchQuery) => {
    if (location.trim()) {
      navigate(
        `/properties?location=${encodeURIComponent(location)}&type=${propertyType}`
      );
    }
  };
            ...
            >
              ...
              <motion.div variants={itemVariants} className="space-y-6">
                <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold leading-tight">
                  <RadialGradient gradient={["circle, rgba(63,94,251,1) 0%, rgba(252,70,107,1) 100%"]}>
              ...
              <motion.div
                variants={containerVariants}
                className="relative z-0 grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 max-w-5xl mx-auto mt-6 md:mt-8"
              >
                {stats.map((stat, index) => (
                  ...
                  >
                    <div
                      className={`w-12 h-12 md:w-14 md:h-14 mx-auto mb-3 md:mb-4 bg-gradient-to-br ${stat.color} rounded-2xl flex items-center justify-center`}
                    >

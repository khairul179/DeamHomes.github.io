...
import Property from '../models/propertymodel.js'; // <-- add this

export const listProducts = async (req, res) => {
  try {
    const limit = Number(req.query.limit) || 50;
    const docs = await Property.find().sort({ createdAt: -1 }).limit(limit);
    // Match frontend expectation: { success: true, property: [...] }
    res.json({ success: true, property: docs });
  } catch (error) {
